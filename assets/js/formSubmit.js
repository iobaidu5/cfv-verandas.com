jQuery(document).ready(function ($) {
    ...
    $("#recrutementForm").on("submit", function (e) {
      sendFormData(e, "#recrutementForm", "recrutement");
    });
    ...
    function sendFormData(e, id, type) {
      ...
      $.ajax({
        ...
        success: function (data, response, message) {
          ...
          if (type === "recrutement"){
          e.target.querySelector("#recrutement-form-response").classList.add("success");
          e.target.querySelector(
            "#recrutement-form-response"
          ).innerHTML = successMessage;
          }
          window.location.href="/message-envoye";  
        ...
        error: function (jqXHR, textStatus, errorThrown) {
          ...
          if (type === "recrutement") {
            e.target.querySelector("#recrutement-form-response").classList.add("error");
            e.target.querySelector(
              "#recrutement-form-response"
            ).innerHTML = ErrorMessage;
            console.log(textStatus);
  ...
  var CaptchaCallback = function () {
    jQuery(".g-recaptcha").each(function () {
      grecaptcha.render(this, {
        sitekey: "6LeaqxgpAAAAACWhpj477_Hj7Ym-Gdx9tIsVSoHE",
